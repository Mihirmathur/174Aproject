...
Declare_Any_Class("Pyramid", {
    'populate' (using_flat_shading) {
        ...
        this.positions.push(vec3(-1, 0, -1), vec3(-1, 0, 1), vec3(1, 0, 1));
        this.positions.push(vec3(-1, 0, -1), vec3(0, Math.sqrt(2), 0), vec3(1, 0, -1));
        this.positions.push(vec3(-1, 0, 1), vec3(0, Math.sqrt(2), 0), vec3(1, 0, 1));
        this.positions.push(vec3(-1, 0, -1), vec3(0, Math.sqrt(2), 0), vec3(-1, 0, 1));
        this.positions.push(vec3(1, 0, -1), vec3(0, Math.sqrt(2), 0), vec3(1, 0, 1));
...
Declare_Any_Class("Windmill", // As our shapes get more complicated, we begin using matrices and flow control (including loops) to
    {
        'populate' (num_blades) // generate non-trivial point clouds and connect them.
        {
            for (var i = 0; i < num_blades; i++) // A loop to automatically generate the triangles.
            {
                ...
                this.indices.push(3 * i + 2); // Procedurally connect the three
...
Declare_Any_Class("Surface_Of_Revolution", {
    populate(rows, columns, points, total_curvature_angle = 360, texture_coord_range = [
        [0, 1][0, 1]
    ]) {
...
Declare_Any_Class("Cylindrical_Tube", // An open tube shape with equally sized sections, pointing down Z locally.    
    {
        populate(rows, columns) { Surface_Of_Revolution.prototype.insert_transformed_copy_into(this, [rows, columns, [vec3(1, 0, .5), vec3(1, 0, -.5)]]); }
    }, Shape)

Declare_Any_Class("Cone_Tip", // Note:  Curves that touch the Z axis degenerate from squares into triangles as they sweep around
    {
        populate(rows, columns) { Surface_Of_Revolution.prototype.insert_transformed_copy_into(this, [rows, columns, [vec3(0, 0, 1), vec3(1, 0, -1)]]); }
    }, Shape)
...
Declare_Any_Class("Rounded_Closed_Cone", // An alternative without two separate sections
    {
        populate(rows, columns) { Surface_Of_Revolution.prototype.insert_transformed_copy_into(this, [rows, columns, [vec3(0, 0, 1), vec3(1, 0, -1), vec3(0, 0, -1)]]); }
    }, Shape)
