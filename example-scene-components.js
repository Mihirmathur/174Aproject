...
Declare_Any_Class("Bee_Scene", // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
{
    ...
    'display' (graphics_state) {
            ...
            this.shapes.square.draw(graphics_state, model_transform, this.brown);

            model_transform = identity();
            model_transform = identity();

            model_transform = mult(model_transform, translation(0, -(8 * 2), 0));

            //Stem
            for (var i = 0; i < 8; i++) {
                var stemangle = 3 * Math.cos(t + Math.PI / 2);
                //model_transform = mult(model_transform, translation(0, 0, 0));
                model_transform = mult(model_transform, rotation(stemangle, [0, 0, 1]));
                model_transform = mult(model_transform, translation(0, 2, 0));
            ...
            model_transform = mult(model_transform, rotation(-stemangle * 8, [0, 0, 1]));
            model_transform = mult(model_transform, scale(3, 3, 3));
            this.shapes.head.draw(graphics_state, model_transform, this.red);

            //BEE
            model_transform = identity();

            //Animations on whole body
            model_transform = mult(model_transform, rotation((100 * t), [0, 1, 0]));
            model_transform = mult(model_transform, translation(12, 6 * (Math.sin(2*t)), 0));

            //Head
            this.shapes.head.draw(graphics_state, model_transform, this.yellow);

            //Midsection
            model_transform = mult(model_transform, translation(0, 0, 3));
            model_transform = mult(model_transform, scale(1, 1, 2));
            this.shapes.box.draw(graphics_state, model_transform, this.mustard);
            ...
            model_transform = mult(model_transform, rotation((20 * (Math.sin(2 * t))), [0, 0, 1]));
            model_transform = mult(model_transform, translation(2.8, 1, 0));
            model_transform = mult(model_transform, scale(2, .1, 2));
            //model_transform = mult(model_transform, translation(-3.465, 0, 0));
            ...
            //Function for generating one leg;
            var right = 0;
            this.draw_leg = function() {
                st.push(model_transform);
                //model_transform = identity();
                model_transform = mult(model_transform, translation(0, 1, 0));
                if (right == 1) {
                    model_transform = mult(model_transform, rotation((30 * (Math.sin(3 * t))), [0, 0, 1]));
                } else {
                    model_transform = mult(model_transform, rotation((-30 * (Math.sin(3 * t))), [0, 0, 1]));
                }
                model_transform = mult(model_transform, translation(0, -1, 0));
                ...
                if (right == 1) {
                    model_transform = mult(model_transform, translation(-.30, -1.7, 0));
                    model_transform = mult(model_transform, rotation(-20, [0, 0, 1]));

                } else {
                    model_transform = mult(model_transform, translation(.3, -1.7, 0));
                    model_transform = mult(model_transform, rotation(20, [0, 0, 1]));
            ...
            model_transform = model_stack.pop();
            model_stack.push(model_transform);
            model_transform = mult(model_transform, translation(-1, -2, .5));
            this.draw_leg();
            ...
            this.shapes.head.draw(graphics_state, model_transform, this.blackish);
        }
    }, Scene_Component);
...
Declare_Any_Class("Example_Camera", // An example of a Scene_Component that our Canvas_Manager can manage.  Adds both first-person and
{
        ...
        { // 1st parameter below is our starting camera matrix.  2nd is the projection:  The matrix that determines how depth is treated.  It projects 3D points onto a plane.
            ...
            canvas.addEventListener("mouseout", (function(self) {
                return function(e) { self.mouse.from_center = vec2(); };
            })(this), false); // Stop steering if the 
...
Declare_Any_Class("Body", {
    ...
    'randomize' (s, m) {
        this.define_data_members({
            ...
            angular_velocity: .5 * Math.random(),
            spin_axis: random_vec3(1),
            material: m
        })
    },
...
Declare_Any_Class("Simulation_Scene_Superclass", {
    ...
    'random_shape' () {
        return Object.values(this.shapes)[Math.floor(7 * Math.random())]
    },
    'random_material' () {
        return this.shader.material(Color(1, Math.random(), Math.random(), 1), .1, 1, 1, 40, this.stars)
    },
    'display' (graphics_state) {
